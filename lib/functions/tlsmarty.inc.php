...
class TLSmarty extends Smarty {
  ...
  function __construct() {
    ...
    if(!is_null($tlCfg->custom_css) && $tlCfg->custom_css != '') {
      ...
      $css_file = $_SERVER['DOCUMENT_ROOT'] . '/testlink/' . TL_THEME_CSS_DIR . $tlCfg->custom_css;
      $this->assign('custom_css', 
        $basehref . TL_THEME_CSS_DIR . $tlCfg->custom_css . '?v=' . (file_exists($css_file) ? filemtime($css_file) : '1'));
    }
